...
- append-only update/status/media histories with conservative availability semantics, field-level raw lineage, point-in-time readers, and immutable SQLite persistence;
...
`docs/spec/listing-histories.md` owns append-only update/status/media semantics. The current implementation consumes only already-proven `get_games` and `__playPageData__` fields; it does not add a scheduler or infer lifecycle states from transport failures.

`YandexListingHistoryNormalizer@1` preserves `publishedTime` as `source_published_at`, records public presence conservatively as `published`, records a known requested ID omitted from a successful catalogue response only as `unknown`, and fingerprints the exact parsed media manifest with canonical SHA-256. `YandexGetGamesParser@4` preserves missing `media` separately from a present empty `{}` manifest.
