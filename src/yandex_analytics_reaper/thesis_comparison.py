...
class ThesisIntelligencePayload(BaseModel):
    ...
    def validate_semantics(self) -> Self:
        ...
        if self.anomaly_policy is None:
            ...
            evaluation_ids = tuple(item.platform_listing_id for item in self.anomaly_evaluations)
        ...
        if self.competitor_quality.review_artifact_present != review_present:
            raise ValueError("review hash presence must match competitor-quality review presence")
        return self
...
class ThesisComparisonPayload(BaseModel):
    ...
    def validate_shape(self) -> Self:
        if len(self.thesis_report_hashes) != len(self.rows):
            raise ValueError("comparison thesis report hashes must align one-to-one with rows")
...
def build_thesis_intelligence_reports(
    ...
    *,
    ...
    if current.binding != traction.current_experiment:
        raise ThesisIntelligenceError("current evidence binding does not equal traction binding")
    if (
        ...
        raise ThesisIntelligenceError("anomaly report does not bind supplied suite/traction")
    ...
    for thesis in suite.theses:
        ...
        report = _build_one_thesis_report(
            suite=suite,
            thesis=thesis,
            traction=traction,
...
def build_thesis_comparison(
    ...
    *,
    ...
    suite = ThesisSuiteDeclaration.model_validate(suite.model_dump())
    reports = tuple(validate_thesis_intelligence_report(item) for item in thesis_reports)
    expected_ids = tuple(item.thesis_id for item in suite.theses)
    actual_ids = tuple(item.thesis_id for item in reports)
    if actual_ids != expected_ids:
        raise ThesisIntelligenceError("thesis report order/identity does not match suite")
    ...
    if len(current_hashes) != 1:
        raise ThesisIntelligenceError("thesis reports do not share one current experiment artifact")
    canonical_prior_hashes = tuple(item.artifact_sha256 for item in reports[0].prior_experiments)
    for report in reports:
        if tuple(item.artifact_sha256 for item in report.prior_experiments) != canonical_prior_hashes:
            raise ThesisIntelligenceError("thesis reports do not share canonical prior artifacts")
    ...
    for thesis, report in zip(suite.theses, reports, strict=True):
        ...
        if report.semantic_report_content_hash != semantic.content_hash:
            raise ThesisIntelligenceError("thesis report semantic hash does not match supplied report")
        review = review_by_id.get(thesis.thesis_id)
        if (None if review is None else review.content_hash) != report.review_content_hash:
            raise ThesisIntelligenceError("thesis report review hash does not match supplied review")
    ...
    return validate_thesis_comparison(report, suite=suite, thesis_reports=reports)
...
def validate_thesis_intelligence_report(
    ...
    if report.content_hash != canonical_model_hash(payload):
        raise ThesisIntelligenceError("thesis intelligence report content_hash mismatch")
    return report
...
def validate_thesis_comparison(
    ...
    *,
    ...
    if suite is not None:
        suite = ThesisSuiteDeclaration.model_validate(suite.model_dump())
        expected = tuple((item.thesis_id, item.thesis_version, item.label) for item in suite.theses)
        actual = tuple((item.thesis_id, item.thesis_version, item.label) for item in report.rows)
        if (
            report.suite_id != suite.suite_id
            or report.suite_version != suite.suite_version
            or actual != expected
        ):
            raise ThesisIntelligenceError("comparison rows do not match suite declaration order")
    if thesis_reports is not None:
        reports = tuple(validate_thesis_intelligence_report(item) for item in thesis_reports)
        if report.thesis_report_hashes != tuple(item.content_hash for item in reports):
            raise ThesisIntelligenceError("comparison thesis report hashes do not match reports")
    return report


def write_thesis_intelligence_json(report: ThesisIntelligenceReport, path: Path) -> None:
    report = validate_thesis_intelligence_report(report)
    _write_create_only(path, report.model_dump_json(indent=2) + "\n")


def write_thesis_intelligence_csv(report: ThesisIntelligenceReport, path: Path) -> None:
    report = validate_thesis_intelligence_report(report)
    rows = []
    evaluations = {item.platform_listing_id: item for item in report.anomaly_evaluations}
        ...
        rows.append(
            {
                ...
                "observed_rating_delta_per_day": row.longitudinal.observed_rating_delta_per_day,
                "anomaly_candidate": row.platform_listing_id in candidates,
                "anomaly_max_age": None if evaluation is None else evaluation.max_age_days_status,
                "anomaly_min_rating_count": (
                    None if evaluation is None else evaluation.min_rating_count_status
                ),
                ...
                "anomaly_min_age_bucket_percentile": (
                    None if evaluation is None else evaluation.min_age_bucket_percentile_status
                ),
...
def write_thesis_comparison_json(report: ThesisComparisonReport, path: Path) -> None:
    report = validate_thesis_comparison(report)
    _write_create_only(path, report.model_dump_json(indent=2) + "\n")


def write_thesis_comparison_csv(report: ThesisComparisonReport, path: Path) -> None:
    report = validate_thesis_comparison(report)
    rows = []
    for row in report.rows:
        record = row.model_dump(mode="json", exclude={
            "best_confirmed_direct_rating_count",
            "best_confirmed_direct_lifetime_pace",
            "best_adjacent_rating_count",
        })
        _flatten_highlight(record, "best_confirmed_direct_rating_count", row.best_confirmed_direct_rating_count)
        _flatten_highlight(
            record,
            "best_confirmed_direct_lifetime_pace",
            row.best_confirmed_direct_lifetime_pace,
        )
        _flatten_highlight(record, "best_adjacent_rating_count", row.best_adjacent_rating_count)
        rows.append(record)
    _write_csv_create_only(path, rows)


def write_thesis_comparison_markdown(report: ThesisComparisonReport, path: Path) -> None:
    report = validate_thesis_comparison(report)
    lines = [
        "# Thesis comparison",
        "",
        ...
        "",
        "| Thesis | Raw | Direct | Confirmed | Fresh confirmed <=180d | Anomalies | Semantic coverage | Longitudinal coverage | Mean query Jaccard |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in report.rows:
        lines.append(
...
def _build_one_thesis_report(
    *,
    ...
    if anomaly_queue.status == "enabled":
        evaluation_ids = tuple(item.platform_listing_id for item in anomaly_queue.evaluations)
        if evaluation_ids != traction_ids:
            raise ThesisIntelligenceError("anomaly evaluations do not follow thesis traction order")
    candidate_ids = tuple(item.platform_listing_id for item in anomaly_queue.candidates)
...
def _comparison_row(
    ...
    traction_by_id = {item.platform_listing_id: item for item in report.traction}
    order = {item.platform_listing_id: index for index, item in enumerate(report.traction)}
    ...
    fresh_confirmed = sum(
        traction_by_id[listing_id].listing_age_days is not None
        and traction_by_id[listing_id].listing_age_days <= 180.0
        for listing_id in confirmed_ids
    )
    age_observed = [item for item in report.traction if item.listing_age_days is not None]
    recent = sum(item.listing_age_days <= 180.0 for item in age_observed)
    recent_share = None if not age_observed else recent / len(age_observed)
    recent_coverage = len(age_observed) / len(report.traction)
    ...
    return ThesisComparisonRow(
        ...
        mean_pairwise_query_jaccard=quality.query_surface.mean_pairwise_jaccard,
...
def _best_highlight(
    ...
    *,
    ...
    for listing_id in listing_ids:
        ...
        if row is None:
            raise ThesisIntelligenceError("highlight listing is outside thesis comparable")
        raw_value = (
            row.rating_count
    ...
    value = (
        float(best.rating_count)
        if metric_name == "rating_count" and best.rating_count is not None
        else best.lifetime_ratings_per_day
    )
    if value is None:
        raise ThesisIntelligenceError("selected highlight unexpectedly lacks metric value")
    observation_id = best.rating_count_observation_id if metric_name == "rating_count" else None
...
def _semantic_reports_by_thesis(
    ...
    for report in reports:
        ...
        if thesis_id not in expected:
            raise ThesisIntelligenceError("semantic report references thesis outside suite")
        ...
        if report.thesis != compiled[thesis_id]:
            raise ThesisIntelligenceError("semantic report declaration does not equal suite compile")
        by_id[thesis_id] = report
    if set(by_id) != expected:
        raise ThesisIntelligenceError("one semantic report is required for every suite thesis")
    return by_id
...
def _reviews_by_thesis(
    ...
    for review in reviews:
        ...
        if thesis_id not in semantics:
            raise ThesisIntelligenceError("review references thesis outside semantic report set")
        if thesis_id in by_id:
...
def _traction_sets_by_thesis(
    ...
    expected = tuple((item.thesis_id, item.thesis_version) for item in suite.theses)
    actual = tuple((item.thesis_id, item.thesis_version) for item in traction.theses)
    if actual != expected:
...
def _anomaly_queues_by_thesis(
    ...
    expected = tuple((item.thesis_id, item.thesis_version) for item in suite.theses)
    actual = tuple((item.thesis_id, item.thesis_version) for item in anomaly.theses)
    if actual != expected:
        raise ThesisIntelligenceError("anomaly thesis order/identity does not match suite")
...
def _validate_highlight_metric(
    ...
    if highlight is not None and highlight.metric_name != metric_name:
        raise ValueError("comparison highlight metric name does not match field")
...
def _flatten_highlight(
    ...
    record[f"{prefix}_listing_id"] = None if highlight is None else highlight.platform_listing_id
    ...
    record[f"{prefix}_observation_id"] = None if highlight is None else highlight.observation_id


def _write_csv_create_only(path: Path, rows: list[dict[str, object]]) -> None:
    if not rows:
