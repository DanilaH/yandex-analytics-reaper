...
def load_experiment_artifact_binding(
    ...
    *,
    ...
    """Verify one immutable 0.2 experiment ZIP and reconstruct its 0.3 binding."""
    ...
    if packaged.experiment_id != manifest.experiment_id:
        raise ThesisIntelligenceError("artifact manifest experiment_id disagrees with input manifest")
    if summary.experiment_id != manifest.experiment_id:
        raise ThesisIntelligenceError("execution summary experiment_id disagrees with input manifest")
    if packaged.run_id != summary.run_id:
        raise ThesisIntelligenceError("artifact manifest run_id disagrees with execution summary")
    if summary.manifest_sha256 != manifest_sha256:
        raise ThesisIntelligenceError("execution summary manifest hash disagrees with input manifest")
    ...
    if summary_hashes != expected_hashes:
        raise ThesisIntelligenceError("execution summary report hashes disagree with report content")
    if verification_hashes != expected_hashes or verification.status != "pass":
        raise ThesisIntelligenceError("experiment verification report disagrees with report content")
    if snapshot.snapshot_id != market_export.snapshot_id:
        raise ThesisIntelligenceError("market export snapshot_id disagrees with analyst snapshot")
    if snapshot.content_hash != market_export.snapshot_content_hash:
        raise ThesisIntelligenceError("market export snapshot hash disagrees with analyst snapshot")
    if snapshot.snapshot_id != market_features.snapshot_id:
        raise ThesisIntelligenceError("market features snapshot_id disagrees with analyst snapshot")
    if snapshot.content_hash != market_features.snapshot_content_hash:
        raise ThesisIntelligenceError("market features snapshot hash disagrees with analyst snapshot")

    if expected_suite is not None:
        compiled = compile_thesis_suite(expected_suite)
        if manifest != compiled.experiment_manifest:
            raise ThesisIntelligenceError(
                "current experiment manifest does not equal the suite-compiled experiment manifest"
            )
